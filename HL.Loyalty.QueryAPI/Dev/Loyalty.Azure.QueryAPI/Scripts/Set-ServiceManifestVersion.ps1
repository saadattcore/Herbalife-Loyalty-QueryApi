...
function Set-ServiceManifestVersion
{
    ...
    foreach ($file in $Files)
    {
		...
		Write-Verbose -Message ("Setting ServiceManifest.Version to " + $newVersion) -verbose
		Write-Verbose -Message ("Setting ServiceManifest.CodePackage.Version to " + $newVersion) -verbose
...
function Set-ApplicationManifestVersion{
	...
	$xml.ApplicationManifest.ApplicationTypeVersion = $newVersion
		Write-Verbose -Message ("Setting ApplicationManifest.ApplicationTypeVersion to " + $newVersion)-verbose

	foreach($serviceReference in $xml.ApplicationManifest.ServiceManifestImport)
	{
		$serviceReference.ServiceManifestRef.ServiceManifestVersion = $newVersion		
		Write-Verbose -Message ("Setting ApplicationManifest.ServiceManifestImport.ServiceManifestRef.ServiceManifestVersion "+$serviceReference.ServiceManifestRef.ServiceManifestName+" to " + $newVersion) -verbose
	}
